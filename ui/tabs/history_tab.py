...
class HistoryTab(QWidget):
    ...
    def init_ui(self):
        """Initialize UI - styles inherited from theme_config"""
        ...
        # Search box - inherits from theme
        ...
        # Sync Now button - keep green for visibility
        ...
        # History table - inherits from theme_config
    ...
    def sync_now(self):
        ...
        try:
            ...
            QMessageBox.information(self, "Success", "History synced!")
        except Exception as e:
    ...
    def export_history(self):
        ...
        csv_btn.setStyleSheet(f"background-color: {colors['success']};")
        csv_btn.clicked.connect(lambda: self._do_export(dialog, "csv"))
        btn_layout.addWidget(csv_btn)
        
        pdf_btn = QPushButton("📕 PDF")
        pdf_btn.setStyleSheet(f"background-color: {colors['danger']};")
    ...
    def export_to_csv(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(
            self, "Export to CSV", "translation_history.csv", "CSV Files (*.csv)"
        )
        ...
        if filename:
            try:
                ...
                QMessageBox.information(self, "Success", f"Đã xuất {len(self.current_history)} bản ghi")
    ...
    def export_to_pdf(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(
            self, "Export to PDF", "translation_history.pdf", "PDF Files (*.pdf)"
        )
        ...
        try:
            ...
            style_body = ParagraphStyle('Body', fontName=font_name, fontSize=9, leading=12)
            ...
            for idx, item in enumerate(self.current_history, 1):
                ...
                data.append([
                    str(idx), time_str,
                    Paragraph(item.get('sourceText', '')[:100], style_body),
                    Paragraph(item.get('translatedText', '')[:100], style_body),
                    lang, item.get('modelUsed', '')
                ])
